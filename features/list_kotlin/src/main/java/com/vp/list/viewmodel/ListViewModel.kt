...
class ListViewModel @Inject internal constructor(val searchService: SearchService) : ViewModel() {
        ...
        searchService.search(title, page).enqueue(object : Callback<SearchResponse> {
            override fun onResponse(call: Call<SearchResponse?>, response: Response<SearchResponse?>) {
                ...
                if (result?.search != null) {
                    liveData.setValue(SearchResult.success(result.search!!, result.totalResults))
                }
            }
