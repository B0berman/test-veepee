...
class DetailsViewModel @Inject constructor(private val detailService: DetailService, private val application: Application) : ViewModel() {
    ...
    fun fetchDetails() {
        ...
        if(DetailActivity.queryProvider != null){
    ...
    fun saveFavorite(){
        thread {
            ...
            if(detailsValue != null && DetailActivity.queryProvider != null){
    ...
    fun getFavorite() : LiveData<List<FavoriteEntity>>? {
        return if (DetailActivity.queryProvider != null){
            AppDatabase.getDatabase(application)?.favoriteDao()?.isFavorite(DetailActivity.queryProvider!!.getMovieId())
        } else {
            null
        }
    }
