...
import com.vp.list.ListAdapter.Companion.LOADING
...
class ListFragment : Fragment(), GridPagingScrollListener.LoadMoreItemsListener, ListAdapter.OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            val savedQuery = savedInstanceState.getString(CURRENT_QUERY)
            if (savedQuery != null) {
        ...
        errorButton.setOnClickListener { loadData(currentQuery) }
    }
        ...
        layoutManager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                return when (listAdapter.getItemViewType(position)) {
                    LOADING -> if (resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT) 2 else 3
                    else -> 1
                }
            }
        }
    ...
    private fun showError() {
        viewAnimator.displayedChild = viewAnimator.indexOfChild(errorLayout)
    }
    ...
    override fun onItemClick(imdbID: String?) {
        // When the imdbID is null or empty, we can be generate a Toast, Snackbar, logs
        if (imdbID != null && !imdbID.isEmpty()) {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("app://movies/detail?imdbID=$imdbID"))
            intent.setPackage(requireContext().packageName)
            startActivity(intent)
        } else {
            Toast.makeText(context, R.string.movies_not_found, Toast.LENGTH_SHORT).show()
        }
    }
