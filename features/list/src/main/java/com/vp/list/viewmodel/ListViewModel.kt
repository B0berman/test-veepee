...
class ListViewModel @Inject internal constructor(private val searchService: SearchService) : ViewModel() {
    ...
    fun searchMoviesByTitle(title: String, page: Int, isRefresh: Boolean = false) {

        if (page == 1 && title != currentTitle || isRefresh) {
        ...
        searchService.search(title, page)!!.enqueue(object : Callback<SearchResponse?> {
            override fun onResponse(call: Call<SearchResponse?>, response: Response<SearchResponse?>) {
                response.body()?.let { searchResponse ->
                    searchResponse.search?.let { aggregatedItems.addAll(it) }
                    liveData.value = SearchResult.success(aggregatedItems, searchResponse.totalResults)
                }
            }
